...
local myConfig = ConfigManager:CreateConfig("ZyrenConfig")
...
-------------------------------------------
----- =======[ COORDINATE DISPLAY GUI ]
-------------------------------------------

-- Buat GUI untuk menampilkan koordinat
...
PositionTitle.Text = "POSITION (Vector3):"
...
local PositionTextCorner = Instance.new("UICorner")
PositionTextCorner.CornerRadius = UDim.new(0, 4)
PositionTextCorner.Parent = PositionText

-- CFrame Display
local CFrameTitle = Instance.new("TextLabel")
CFrameTitle.Name = "CFrameTitle"
CFrameTitle.Size = UDim2.new(1, 0, 0, 20)
CFrameTitle.Position = UDim2.new(0, 0, 0, 60)
CFrameTitle.BackgroundTransparency = 1
CFrameTitle.Text = "CFRAME:"
CFrameTitle.TextColor3 = Color3.fromRGB(200, 200, 255)
CFrameTitle.TextSize = 12
CFrameTitle.TextXAlignment = Enum.TextXAlignment.Left
CFrameTitle.Font = Enum.Font.Gotham
CFrameTitle.Parent = ContentFrame

local CFrameText = Instance.new("TextLabel")
CFrameText.Name = "CFrameText"
CFrameText.Size = UDim2.new(1, 0, 0, 50)
CFrameText.Position = UDim2.new(0, 0, 0, 80)
CFrameText.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
CFrameText.BorderSizePixel = 0
CFrameText.Text = "CFrame.new(0, 0, 0)"
CFrameText.TextColor3 = Color3.fromRGB(255, 255, 255)
CFrameText.TextSize = 10
CFrameText.TextXAlignment = Enum.TextXAlignment.Left
CFrameText.TextYAlignment = Enum.TextYAlignment.Top
CFrameText.TextWrapped = true
CFrameText.Font = Enum.Font.Gotham
CFrameText.Parent = ContentFrame

local CFrameTextCorner = Instance.new("UICorner")
CFrameTextCorner.CornerRadius = UDim.new(0, 4)
CFrameTextCorner.Parent = CFrameText
...
local CopyPositionCorner = Instance.new("UICorner")
CopyPositionCorner.CornerRadius = UDim.new(0, 4)
CopyPositionCorner.Parent = CopyPositionButton
...
local CopyCFrameCorner = Instance.new("UICorner")
CopyCFrameCorner.CornerRadius = UDim.new(0, 4)
CopyCFrameCorner.Parent = CopyCFrameButton

-- Function untuk update coordinates
local function UpdateCoordinates()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        PositionText.Text = string.format("X: %.2f, Y: %.2f, Z: %.2f", position.X, position.Y, position.Z)
        
        -- Update CFrame Text dengan format yang rapi
        CFrameText.Text = string.format(
            "CFrame.new(\n%.2f, %.2f, %.2f)",
            position.X, position.Y, position.Z
        )
    else
        PositionText.Text = "Character not found"
        CFrameText.Text = "CFrame.new(0, 0, 0)"
    end
end
...
local function CopyToClipboard(text)
    local success, result = pcall(function()
        if setclipboard then
            ...
            -- Fallback untuk executor yang tidak support setclipboard
            return false
        end
    end)
...
CopyPositionButton.MouseButton1Click:Connect(function()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        local positionText = string.format("Vector3.new(%.2f, %.2f, %.2f)", position.X, position.Y, position.Z)
...
CopyCFrameButton.MouseButton1Click:Connect(function()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        local cframeText = string.format("CFrame.new(%.2f, %.2f, %.2f)", position.X, position.Y, position.Z)
        CopyToClipboard(cframeText)
    end
end)

-- Update coordinates secara real-time
game:GetService("RunService").RenderStepped:Connect(function()
    if CoordinateGUI.Enabled then
        UpdateCoordinates()
    end
end)
...
OtherTab:Toggle({
    ...
    Desc = "Display real-time position and CFrame",
    Value = false,
...
OtherTab:Button({
    ...
    Desc = "Copy Vector3 position to clipboard",
    Justify = "Center",
    Icon = "copy",
    Callback = function()
        local character = LocalPlayer.Character
...
OtherTab:Section({
    Title = "Advanced Coordinate Tools",
    TextSize = 22,
    TextXAlignment = "Center",
    Opened = true
})
...
OtherTab:Button({
    ...
    Desc = "Save current position to memory",
    Justify = "Center",
    Icon = "bookmark",
    Callback = function()
        local character = LocalPlayer.Character
        if character and character:FindFirstChild("HumanoidRootPart") then
            ...
            WindUI:Notify({
                ...
                Content = string.format("Position saved! Total: %d", #SavedPositions),
                Duration = 3,
                Icon = "bookmark-check"
            })
        end
...
OtherTab:Button({
    ...
    Desc = "Clear all saved positions",
    Justify = "Center",
    Icon = "trash-2",
    Callback = function()
        SavedPositions = {}
...
OtherTab:Button({
    Title = "Show Saved Positions",
    Desc = "Display all saved positions",
    Justify = "Center",
    Icon = "list",
    Callback = function()
        if #SavedPositions == 0 then
            WindUI:Notify({
                Title = "No Positions",
                Content = "No positions saved yet!",
                Duration = 3,
                Icon = "info"
            })
            return
        ...
        local message = "ðŸ“‹ Saved Positions:\n\n"
        for i, pos in ipairs(SavedPositions) do
            message = message .. string.format("%d. [%s] X:%.1f, Y:%.1f, Z:%.1f\n", 
                i, pos.Time, pos.Position.X, pos.Position.Y, pos.Position.Z)
        end
        
        WindUI:Notify({
            Title = "Saved Positions (" .. #SavedPositions .. ")",
            Content = message,
            Duration = 8,
            Icon = "list"
        })
...
OtherTab:Button({
    Title = "Teleport to Coordinates",
    Desc = "Teleport to specified coordinates",
    Justify = "Center",
    Icon = "navigation",
    Callback = function()
        local x = tonumber(XInput:Get())
        local y = tonumber(YInput:Get())
        local z = tonumber(ZInput:Get())
        ...
        if x and y and z then
            ...
            WindUI:Notify({
                ...
                Content = "Please enter valid numbers for X, Y, Z",
                Duration = 3,
                Icon = "x-circle"
            })
        end
...
print("âœ… Other Tab with Coordinate Display loaded successfully!")
...
_G.REReplicateCutscene.OnClientEvent:Connect(function(rarity, player, position, fishName, data)
    if _G.BlockCutsceneEnabled then
        print("[ZyrenHub] Cutscene diblokir:", fishName, "(Rarity:", rarity .. ")")
