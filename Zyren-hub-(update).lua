...
-- ============================================================================
-- KEY SYSTEM - TARUH DI SINI
-- ============================================================================
local KeySystem = {
    CorrectKeys = {"dirga1708", "tesssss"},
    KeyVerified = false
}

_G.VerifyKey = function(inputKey)
    for _, correctKey in ipairs(KeySystem.CorrectKeys) do
        if inputKey == correctKey then
            KeySystem.KeyVerified = true
            WindUI:Notify({
                Title = "KEY SYSTEM", 
                Content = "✅ Key verified! Other tab unlocked.",
                Duration = 5,
                Icon = "key"
            })
            return true
...
_G.ShowCoordinateDisplay = function()
    -- Hapus GUI lama jika ada
    if game:GetService("CoreGui"):FindFirstChild("CoordDisplayGUI") then
        game:GetService("CoreGui").CoordDisplayGUI:Destroy()
    end
    ...
    CoordDisplay.Size = UDim2.new(1, -20, 0, 90)
    CoordDisplay.Position = UDim2.new(0, 10, 0, 35)
    CoordDisplay.BackgroundColor3 = Color3.fromRGB(20, 20, 30)
    CoordDisplay.TextColor3 = Color3.fromRGB(0, 255, 127)
    CoordDisplay.TextSize = 11
    CoordDisplay.Font = Enum.Font.Code
    CoordDisplay.TextXAlignment = Enum.TextXAlignment.Left
    CoordDisplay.TextYAlignment = Enum.TextYAlignment.Top
    CoordDisplay.Text = "Loading coordinates..."
    CoordDisplay.Parent = MainFrame

    -- Buttons Frame
    local ButtonsFrame = Instance.new("Frame")
    ButtonsFrame.Size = UDim2.new(1, -20, 0, 70)
    ButtonsFrame.Position = UDim2.new(0, 10, 0, 135)
    ButtonsFrame.BackgroundTransparency = 1
    ButtonsFrame.Parent = MainFrame

    -- Copy Position Button
    local CopyPosButton = Instance.new("TextButton")
    CopyPosButton.Size = UDim2.new(0.48, 0, 0, 30)
    CopyPosButton.Position = UDim2.new(0, 0, 0, 0)
    CopyPosButton.BackgroundColor3 = Color3.fromRGB(0, 120, 215)
    CopyPosButton.BorderSizePixel = 0
    ...
    CopyPosButton.Parent = ButtonsFrame

    -- Copy CFrame Button
    local CopyCFrameButton = Instance.new("TextButton")
    CopyCFrameButton.Size = UDim2.new(0.48, 0, 0, 30)
    CopyCFrameButton.Position = UDim2.new(0.52, 0, 0, 0)
    CopyCFrameButton.BackgroundColor3 = Color3.fromRGB(0, 150, 100)
    ...
    CopyCFrameButton.Parent = ButtonsFrame
    ...
    local function updateDisplay()
        ...
        if character and character:FindFirstChild("HumanoidRootPart") then
            currentCFrame = character.HumanoidRootPart.CFrame
            currentPosition = currentCFrame.Position
            ...
            local displayText = string.format(
                ...
                currentPosition.Y, 
                currentPosition.Z,
                tostring(currentCFrame)
            )
            
            CoordDisplay.Text = displayText
        else
            CoordDisplay.Text = "❌ Character not found\nWait for respawn..."
        end
    end

    -- Function to copy position to clipboard
    local function copyPosition()
        if currentPosition then
            local posText = string.format("Vector3.new(%.2f, %.2f, %.2f)", 
                currentPosition.X, currentPosition.Y, currentPosition.Z)
            setclipboard(posText)
            Title.Text = "✅ COPIED POSITION!"
            wait(1.5)
            Title.Text = "🎯 COORDINATE DISPLAY"
        end
    end

    -- Function to copy CFrame to clipboard
    local function copyCFrame()
        if currentCFrame then
            setclipboard("CFrame.new(" .. tostring(currentCFrame) .. ")")
            Title.Text = "✅ COPIED CFRAME!"
            wait(1.5)
            Title.Text = "🎯 COORDINATE DISPLAY"
        end
    end

    -- Function to toggle live update
    local function toggleUpdate()
        isUpdating = not isUpdating
        if isUpdating then
            ToggleButton.Text = "🟢 LIVE UPDATE"
            ToggleButton.BackgroundColor3 = Color3.fromRGB(0, 150, 100)
            updateDisplay() -- Update sekali saat diaktifkan
        else
    ...
    local function closeGUI()
        ...
        WindUI:Notify({
            ...
            Content = "Coordinate display closed!",
            Duration = 3,
            Icon = "map-pin"
        })
    end
    ...
    CopyCFrameButton.MouseButton1Click:Connect(copyCFrame)
    ToggleButton.MouseButton1Click:Connect(toggleUpdate)
    ...
    -- Hover effects untuk tombol lainnya
    local buttons = {CopyPosButton, CopyCFrameButton, ToggleButton}
    for _, button in pairs(buttons) do
        button.MouseEnter:Connect(function()
            if button ~= ToggleButton or isUpdating then
                game:GetService("TweenService"):Create(button, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(100, 100, 110)}):Play()
            end
        end)
        
        button.MouseLeave:Connect(function()
            if button == CopyPosButton then
                button.BackgroundColor3 = Color3.fromRGB(0, 120, 215)
            elseif button == CopyCFrameButton then
    ...
    WindUI:Notify({
        Title = "Coordinate Display",
        Content = "Coordinate display opened!\nDrag the title bar to move.",
        Duration = 5,
        Icon = "map-pin"
    })

    print("🎯 Coordinate Display Loaded from Zyren Hub!")
end
...
-- ============================================================================
-- OTHER TAB (akan dibuat setelah key benar) - TARUH DI SINI
-- ============================================================================
local OtherTab = nil

_G.CreateOtherTab = function()
    if OtherTab then return end
    
    OtherTab = AllMenu:Tab({
        Title = "Other",
        Icon = "cog"
    })
    ...
    OtherTab:Button({
        Title = "Show Coordinate and CFrame",
        Desc = "Display real-time position coordinates",
        Justify = "Center",
        Icon = "map-pin",
        Callback = function()
            _G.ShowCoordinateDisplay()
        end
    })
    ...
    print("🔓 Other Tab unlocked and created!")
end
...
-- ============================================================================
-- KEY INPUT - TARUH DI SINI
-- ============================================================================
local KeyInput = SettingsTab:Input({
    Title = "Enter Key for Other Tab",
    Desc = "Input key to unlock Other features\nValid keys: dirga1708 or tesssss",
    Placeholder = "Enter key here...",
    Callback = function(inputKey)
        if _G.VerifyKey(inputKey) then
            _G.CreateOtherTab()
        end
    end
})
